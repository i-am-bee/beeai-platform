...
variables_table = Table(
    ...
    CheckConstraint(
        (
            ...
            "(model_provider_id IS NOT NULL)::int + "
            "(embedding_provider_id IS NOT NULL)::int + "
            "= 1"
        ),
    ...
    CheckConstraint(
        ...
        "(parent_entity = 'embedding_provider' AND embedding_provider_id IS NOT NULL)",
        name="parent_entity_matches_reference",
    ),
    UniqueConstraint(
        "key",
        "parent_entity",
        func.coalesce("provider_id", "model_provider_id", "embedding_provider_id"),
        name="unique_key_per_entity",
    ),
    # Indexes
...
class SqlAlchemyEnvVariableRepository(IEnvVariableRepository):
    ...
    def _get_parent_column(self, parent_entity: EnvStoreEntity):
        ...
        match parent_entity:
            ...
            case EnvStoreEntity.llm_provider:
                return variables_table.c.llm_provider_id
            case EnvStoreEntity.embedding_provider:
                return variables_table.c.embedding_provider_id
            case _:
