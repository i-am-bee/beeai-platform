...
#### Plain dict
Dictionaries send an empty A2A `Message` and supply the dictionary content as message metadata:
...
Similar to `TaskStatus` but designed for streaming updates. Use this when you need to provide progressive status updates, such as when streaming content to the client.

```python
import uuid
from a2a.types import Message, TextPart, TaskStatusUpdateEvent, TaskState, TaskStatus
...
yield TaskStatusUpdateEvent(
    ...
    status=TaskStatus(
        message=Message(
            message_id=str(uuid.uuid4()), role="agent", parts=[TextPart(text="Please provide some input.")]
        ),
        state=TaskState.input_required,
    ),
)
...
Similar to `Artifact` but designed for artifact generation updates. Use this when progressively generating artifacts and want to provide updates on the generation progress.

```python
import uuid
from a2a.types import Message, TaskArtifactUpdateEvent, Artifact, FilePart, FileWithUri
